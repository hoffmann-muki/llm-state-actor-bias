...
# Run a small-sample calibration for Nigeria, pick per-model thresholds, then run on a larger sample
...
# run the Nigeria pipeline (it will write calibrated results/metrics using the usual filenames)
SAMPLE_SIZE=$SMALL_SAMPLE NGA_POSTPROCESS=1 "$VENV_PY" political_bias_of_llms_nga.py

# canonical metrics file produced by the evaluation step
BASE_METRICS="results/metrics_thresholds_calibrated.csv"
METRICS_FILE="results/metrics_thresholds_calibrated_nga.csv"
THRESH_JSON="results/selected_thresholds_nga.json"

# if the pipeline wrote the generic metrics file, move it to a Nigeria-specific name
if [ -f "$BASE_METRICS" ] && [ ! -f "$METRICS_FILE" ]; then
  mv "$BASE_METRICS" "$METRICS_FILE"
  echo "Moved $BASE_METRICS -> $METRICS_FILE"
fi
...
SAMPLE_SIZE=$LARGE_SAMPLE NGA_POSTPROCESS=1 "$VENV_PY" political_bias_of_llms_nga.py

# calibrated CSV that the apply step reads; prefer country-specific filename
BASE_CAL="results/ollama_results_calibrated.csv"
CALIBRATED_CSV="results/ollama_results_calibrated_nga.csv"
APPLY_OUT_CSV="results/threshold_application_on_large_nga.csv"
